...
class RegisterForm(Form):
    ...
    password1 = forms.CharField(
        ...
        max_length=12,
        validators = [
            RegexValidator(r'((?=.*\d))^.{6,12}$', '太简单了,能不能加点数字'),
            RegexValidator(r'((?=.*[a-zA-Z]))^.{6,12}$', '太简单了,加点字母呗'),
            ],  # 用于对密码的正则验证
        error_messages={
            'required':'密码不输一下你还想注册',
            'min_length':'太短啦,要大于6个字符哈',
            'max_length':'那么长干嘛,12个字符够了'
        }
        )
